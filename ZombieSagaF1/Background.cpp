...
{
	bgCentre = D3DXVECTOR2(textureWidth / 2, textureHeight / 2);

	// Crop texture into required rectangle
	bgRect.left = 0;
	bgRect.right = textureWidth;
	bgRect.top = 0;
	bgRect.bottom = textureHeight;
}
...
void Background::Render(LPD3DXSPRITE spriteBrush, D3DXMATRIX* mat, D3DXVECTOR2 scaling, D3DXVECTOR2 position) {
	...
	D3DXMatrixTransformation2D(mat, NULL, 0.0f , &scaling, &bgCentre, 0.0f, &pos);

	// Set the transformation
	spriteBrush->SetTransform(mat);

	// Draw the sprite
	HRESULT hr = spriteBrush->Draw(texture, &bgRect, NULL, NULL, D3DCOLOR_XRGB(255, 255, 255));
	if (FAILED(hr)) {
